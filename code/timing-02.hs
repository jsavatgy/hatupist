...
renewTableR gui gs t = do
  return ()

renewTableI gui gs t = do
  return ()

renewTwoTables gui gs t = do
  print "Renew Tables R and I"
  renewTableR gui gs t
  renewTableI gui gs t
  return ()

renewTables gui gs t iCur = do
  renewTableS gui gs t
  when (lastShownIv gs /= iCur) (renewTwoTables gui gs t)
  return ()
...
whenEntryChanged gui settings lines gsRef = do
  ...
  newgs <- f gui settings pt gs gsRef
...
whenNotStarted status gui settings pt gs gsRef = do
...
whenCorrect gui settings pt gs gsRef = do
  print "Correct."
  return gs {
    total = (total gs) + 1,
    intervals = addTime
...
whenNewError gui settings pt gs gsRef = do
  print "New Error."
  return gs {
    intervals = addTime
      Error 
...
whenOther status oldStatus gui settings pt gs gsRef = do
  putStrLn ("Other with " ++ (show (status,oldStatus)))
  return gs

latestIvNum ivs = iNum (head ivs)
