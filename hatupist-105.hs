import Graphics.UI.Gtk hiding (Settings,rectangle)
...
menuBarDescr = 
  [("_Tiedosto", 
    [(stockOpen, openFile),
     (stockSelectFont, openFont),
     (stockPreferences, setPreferences),
     (stockAbout, showResultPics),
     (stockQuit, quitProgram)])
  ]

createMenuBar descr gsRef = do 
  bar <- menuBarNew
  mapM (createMenu bar) descr
  return bar
  where
    createMenu bar (name,items) = do 
      ...
      mapM (createMenuItem menu) items
...
createFrame container title fileName = do
  box <- vBoxNew False 0
  ...
  boxPackStart box eventbox PackNatural 0
  widgetModifyBg eventbox StateNormal (toGtkColor white)

  boxPackStart container box PackNatural 10

picName metaName gs = 
  (homeDirectory gs) ++ "/" ++ metaName  ++ ".png"

createImage fileName wInt hInt func values = do
  img <- createImageSurface FormatARGB32 wInt hInt
  let 
    (w,h) = (intToDouble wInt, intToDouble hInt)
  renderWith img (func w h values)
...
settingsTable (Settings a b _ _ c e f g h) = [
  ["Aloitusrivi",            show a],
...
readTuple :: String -> IO (Int,Int)
readTuple s = readIO s
...
timeUTC date = case (parseTimeM True defaultTimeLocale "%Y-%m-%d %H:%M:%S" date) of
